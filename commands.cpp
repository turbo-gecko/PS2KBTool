 ...
 * 
 * Copyright 2019, Gary Hammond, all rights reserved.
 */
...
void displayHelp() {
  ...
  S_HOST.println(F("sfc <on|off>"));
  S_HOST.println(F("           - set XON/XOFF flow control"));
  ...
  S_HOST.println(F("er <address>"));
  S_HOST.println(F("           - read value from EEPROM"));
  S_HOST.println(F("           - address"));
  S_HOST.println(F("ew <address> <value>"));
  S_HOST.println(F("           - write value to EEPROM"));
  S_HOST.println(F("           - address"));
}
...
bool processCommand(const String cmdLine) {
  ...
  if (command.equals("help")) {
    ...
    S_HOST.println(F("Serial: sbr,scd,sld,sfc"));
